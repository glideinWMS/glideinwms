...
If not, stop and correct the %(option)s""" % { "option"       : self.option, })
        ...
        if common.not_writeable(os.path.dirname(self.vdt_location())):
            common.logerr("""You do not have permissions to create the vdt_location option specified: %(dir)s""" % { "dir"    : self.vdt_location(), })
        common.logit(""" CA certificates install starting. The packages that will be installed are:  %(package)s""" % { "package" : self.package, })
    ...
    def create_crontab(self):
        ...
        common.logit("\n... %(user)s crontab entries:" % { "user" : pwd.getpwuid(os.getuid())[0], })
        services_file = "%(vdt_location)s/vdt/services/state" % { "vdt_location" : self.vdt_location(), }
        try:
            fd = open(services_file, 'r')
            lines = fd.readlines()
        except:
            common.logerr("Unable to read VDT services file: %(services_file)s" % { "services_file" : services_file, })
        ...
        common.ask_continue("""\n... the glidein services require that CRL files (*.r0) be present in the certificates directory.  Is it OK to run the script now?""")
...
Suggest you check this out before proceeding:
   %(dir)s""" % { "option" : self.option, "dir"   : self.x509_cert_dir(), })
