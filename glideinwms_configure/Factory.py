...
files and directories can be created correctly""" % self.username())
    ...
    def verify_directories_empty(self):
        ...
        for dir_name in ["monitor", "stage"]:
            subdir = os.path.join(self.glidein.web_location(), dir_name)
            if os.path.isdir(subdir) and len(os.listdir(subdir)) > 0:
                dirs["web_location/%s" % dir_name] = subdir
    ...
    def nbr_of_nested_dirs(self):
        ...
        cnt = 0
        for dir_name in  [self.logs_dir(),
                     self.client_log_dir(),
                     self.client_proxy_dir(),
                     self.glidein.web_location() ]:
            if dir_name.find(self.install_location()) == 0:
                cnt = cnt + 1
        return cnt
    ...
    def validate_use_vofrontend_proxy(self):
        ...
        if option == "y":  # using vofrontend 
            if len(self.x509_proxy()) > 0 or len(self.x509_gsi_dn()) > 0:
...
The x509_proxy and x509_gsi_dn option must be populated.""")
        ...
        dn_in_file = common.get_gsi_dn(gsi_type, proxy_file)
...
""" % { "indent2"     : common.indent(2),
  ...
  "glexec_path" : entry_el['glexec_path'],
        ...
        data = data + """%(indent1)s</entries> """ % { "indent1" : common.indent(1), }
        return data
...
{ "indent4" : common.indent(4),
  ...
  "name"    : is_el['name'],
    ...
    def ask_user(self, ress_entries):
        ...
        for key in ress_keys:
            print "[%s] %s(%s)" % (key.ljust(20), ress_entries[key]['gatekeeper'], ress_entries[key]['rsl'])
    ...
    def get_python_filter(self, py_filter):
        obj = None
        try:
            if len(py_filter) > 0:
                obj = compile(py_filter, "<string>", "eval")
        except Exception:
    ...
    def get_ress_data(self):
        ...
        condor_obj = glideinwms_libs.condorMonitor.CondorStatus(pool_name=self.glidein.ress_host())
        try:
    ...
    def get_bdii_data(self):
        ...
        common.logit("Constraints: %s" % constraint)
        try:
            bdii_obj = glideinwms_libs.ldapMonitor.BDIICEQuery(self.glidein.bdii_host(), additional_filter_str=constraint)
            bdii_obj.load()
    ...
    ### AT
    def validate_glidein_proxy(self):
        pass
...
"""
    ...
    parser.add_option("-i", "--ini", dest="inifile", help="ini file defining your configuration")
