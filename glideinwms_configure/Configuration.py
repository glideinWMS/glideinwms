...
def run_unit_tests(pgm):
    try:
        ...
        tests = {
            "no arguments"                  : [1, pgm],
            "not enough arguments: validate": [1, pgm, "--validate"],
            "not enough arguments: compare" : [1, pgm, "--compare"],
            "invalid argument"              : [1, pgm, "--bad-arg", test_dir + "non-existent-file"],
            
            ##  validate ###
            "validate: good ini"  : [0, pgm, "--validate", test_dir + "config-good.ini"],
            "validate: bad ini"   : [1, pgm, "--validate", test_dir + "config-bad.ini"],
            "validate: no ini"    : [1, pgm, "--validate", test_dir + "non-existent-file"],
            "validate: duplicates": [1, pgm, "--validate", test_dir + "config-w-dup-sections.ini"],
            
            ##  compare ###
            "compare: no difference" : [0, pgm, "--compare", test_dir + "config-good.ini", test_dir + "config-good.ini"],
            "compare: differences"   : [1, pgm, "--compare", test_dir + "config-good.ini", test_dir + "config-good-different.ini"],
            "compare: no ini"        : [1, pgm, "--validate", test_dir + "non-existent-file"],
            "compare: duplicates"    : [1, pgm, "--compare", test_dir + "config-good.ini config-w-dup-sections.ini"],
            
            ##  show-options ###
            "show-options"              : [0, pgm, "--show-options", test_dir + "config-good.ini"],
            "show-options: bad ini"     : [1, pgm, "--show-options", test_dir + "config-bad.ini"],
            "show-options: no ini"      : [1, pgm, "--show-options", test_dir + "non-existent-file"],
            "show-options: duplicates"  : [1, pgm, "--show-options", test_dir + "config-w-dup-sections.ini"],
            
            ##  show-values ###
            "show-values"               : [0, pgm, "--show-values", test_dir + "config-good.ini"],
            "show-values: bad ini"      : [1, pgm, "--show-values", test_dir + "config-bad.ini"],
            "show-values: no ini"       : [1, pgm, "--show-values", test_dir + "non-existent-file"],
            "show-values: duplicates"   : [1, pgm, "--show-values", test_dir + "config-w-dup-sections.ini"],
            }
        #---- run tests -----
        n = 0
...
def main(opts=None):
    try:
        ...
        print
        print "Exception ERROR: %s - %s" % (show_line(), e)
        return 1
    return 0
