...
class Condor(Configuration):
    ...
    def set_daemon_list(self, daemon_list):
    ...
    def x509_proxy(self):
        common.logerr("System error.  Should ONLY be overriden by the VOFrontend")
    ...
    def validate_condor_installation(self):
        file_name = "%s/condor.sh" % self.condor_location()
        if not os.path.isfile(file_name):
            common.logerr("""Condor does not appear to be installed. Cannot locate: %s""" % file_name)
    ...
    def condor_config_daemon_users(self):
        common.logerr("System Error:  The method must be overridden.")
...
""" % (self.local_config_dir())
        ...
        for config_type in types:
            filename = "%s/%s.config" % (self.local_config_dir(), config_type)
            common.logit("    %s" % os.path.basename(filename))
            common.write_file("w", 0644, filename, self.condor_config_data[config_type], SILENT=True)
...
""" % { "condor_location" : self.condor_location(),
    ...
    def __condor_config_gwms_data__(self):
        config_type = "00_gwms_general"
        self.condor_config_data[config_type] += """
######################################################
...
"""
    ...
    def __condor_config_collector_data__(self):
        config_type = "01_gwms_collectors"
        if self.daemon_list.find("COLLECTOR") >= 0:
            self.condor_config_data[config_type] += """
###########################################################
# Collector Data
