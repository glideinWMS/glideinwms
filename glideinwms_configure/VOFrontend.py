...
class VOFrontend(Condor):
    ...
    def nbr_of_nested_dirs(self):
        ...
        cnt = 0
        for dir_name in  [self.logs_dir(), self.glidein.web_location() ]:
...
authentification/authorizaton of the glidein pilots"""
        ...
        if self.factory.use_vofrontend_proxy() == "y" and len(self.glidein_proxy_files()) == 0:
...
""" % { "condor_location" : self.condor_location(), }
    ...
    def get_schedds_via_condor_config_val(self):
        ...
        for line in lines:
            ...
            if line != "":
                schedds.append("%(line)s@%(hostname)s" % {"line" : line, "hostname" : self.hostname(), })
...
{ "indent2" : common.indent(2),
  ...
  "match_string"       : glideinwms_libs.xmlFormat.xml_quoteattr(self.match_string()),
...
%(indent5)s<match_attrs> """ % \
 { "indent4" : common.indent(4),
   ...
   "expr"    : glideinwms_libs.xmlFormat.xml_quoteattr(string.join(attr_query_arr, " && ")), }
...
{ "indent1"           : common.indent(1),
  ...
  "job_constraints"   : glideinwms_libs.xmlFormat.xml_quoteattr(self.userjob_constraints()),
}
...
%(indent1)s<files>
%(indent2)s</files>
""" % { "indent1" : common.indent(1),  "indent2" : common.indent(2), }
