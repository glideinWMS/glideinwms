...
class GlideinWMSDistro:
    ...
    class __impl:
        ...
        def __init__(self, distro_dir, chksumFile=None):
            self.versionIdentifier = 'GLIDEINWMS_VERSION'
            if chksumFile == None:
                self.distroChksumFile = os.path.join(distro_dir, 'etc/checksum')
            else:
                self.distroChksumFile = chksumFile
            try:
                self.createVersionString(distro_dir)
            except:
                self._version = 'glideinWMS UNKNOWN'

        def createVersionString(self, distro_dir):
            ver = 'UNKNOWN'
            patch = ""
            modifiedFiles = []
            ...
            try:
                ...
                for line in distroChksumFd.readlines():
                    if not line.strip().startswith('#'):
                        local_file = os.path.normpath(((line.split('  '))[1]).strip())
                        local_hash = ((line.split('  '))[0]).strip()
                        distroFileHash[local_file] = local_hash
                    else:
            ...
            if ver != 'UNKNOWN':
                ...
                for local_file in distroFileHash.keys():
                    fd = None
                    try:
                        fd = open(os.path.join(distro_dir, local_file), 'r')
                        chksum = md5.new(fd.read()).hexdigest()
                        if (chksum != distroFileHash[local_file]):
                            modifiedFiles.append(local_file)
                            patch = 'PATCHED'
                    finally:
                        if fd:
                            fd.close()
    ...
    def __init__(self, distro_dir, chksumFile=None):
        if GlideinWMSDistro.__instance is None:
            GlideinWMSDistro.__instance = GlideinWMSDistro.__impl(distro_dir, chksumFile=chksumFile)
...
def version(distro_dir, chksumFile=None):
    return GlideinWMSDistro(distro_dir, chksumFile=chksumFile).version()
#   version
