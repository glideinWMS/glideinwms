...
def rawTime2cTime(time_str, year):
    ...
    """
    try:
        ctime = time.mktime((year, int(time_str[0:2]), int(time_str[3:5]), int(time_str[6:8]), int(time_str[9:11]), int(time_str[12:14]), 0, 0, -1))
    except ValueError:
        return -1 #invalid
    return ctime
...
def rawTime2cTimeLastYear(time_str):
    ...
    if ctime <= now:
        ...
        ctime = rawTime2cTime(time_str, current_year - 1)
        return ctime
