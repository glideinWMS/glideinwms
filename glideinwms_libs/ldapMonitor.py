...
class SearchBDII:
        ...
        if self.searchStr:
            ...
            for key in self.bdiiData.keys():
                if self.searchStr in key:
                    search_dict[key] = self.bdiiData[key]
                    found = True
            if found == False:
                print "\n No entry found for search term \"" + self.searchStr + "\" .\n"
            self.bdiiData.clear()
            self.bdiiData = search_dict
    ...
    def display_bdii_data(self, output_file=None):
        """Display entries.
        Args (output_file=None): output_file is the specified output file name. If blank, output is on command line.
        """
        if file:
            try:
...
class BdiiLdap:
    ...
    def runldapquery(self, query_filter, attribute):
        bdii = self.bdii
        if self.DEBUG:
            print "runldapquery [" + bdii + "]", query_filter, attribute
        attribute = attribute.split(' ')
        query_filter = query_filter.strip()
        query_filter = query_filter.lstrip("'").rstrip("'")
        ...
        try:
            result_id = l.search(base, scope, query_filter, attribute)
            while 1:
    ...
    def buildOrQuery(self, gluekey, query_list):
        """
