...
class BaseRRDSupport:
    ...
    #############################################################
    def update_rrd(self, rrdfname, rrdtime, val):
        """
        Create an RRD archive with a new value

        Arguments:
          rrdfname - File path name of the RRD archive
          rrdtime     - When was the value taken
        ...
        lck = self.get_disk_lock(rrdfname)
        try:
            self.rrd_obj.update(str(rrdfname), '%li:%s' % (rrdtime, val))
        finally:
            lck.close()
    ...
    #############################################################
    def update_rrd_multi(self, rrdfname, rrdtime, val_dict):
        """
        ...
        args.append(('%li:' % rrdtime) + ':'.join(ds_vals))
