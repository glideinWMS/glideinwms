...
class EnvState:
    ...
    def load(self):
        ...
        saved_state = {}
        for condor_key in state_filter:
            env_key = "_CONDOR_%s" % condor_key
...
def convert_sec_filter(sec_filter):
    ...
    for context in sec_filter.keys():
        for feature in sec_filter[context]:
            ...
            local_filter.append(condor_key)
    return local_filter
...
class SecEnvRequest:
    ...
    def save_state(self):
        ...
        for c in self.requests.keys():
            local_filter[c] = self.requests[c].keys()

        self.saved_state = SecEnvState(local_filter)
        ...
        if state_filter != None:
            ...
            for c in CONDOR_CONTEXT_LIST:
                ...
                for f in CONDOR_PROTO_FEATURE_LIST:
                    cfilter.append(f)
                state_filter[c] = cfilter

        SecEnvState.__init__(self, state_filter)
