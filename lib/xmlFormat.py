...
def xml_quoteattr(el):
    if el is None:
        val = '"None"'
    elif type(el) in (str, str):  # May need to add bytes depending on Python3
...
def class2head(inst, inst_name, params, dicts_params, lists_params, tree_params, text_params, leading_tab, debug_str):
    ...
    if (
        (len(inst_attrs) == 0)
        and (len(dict_attrs) == 0)
        and (len(list_attrs) == 0)
        and (len(tree_attrs) == 0)
        and (len(text_attrs) == 0)
    ):
        head_arr.append("/>")
        is_complete = 1
    else:
...
def class2string(
    ...
    tree_params=None,
    text_params=None,
    indent_tab=DEFAULT_TAB,
    leading_tab="",
    debug_str="",
    override_dictionary_type=None,
):
...
def tree2string(tree, tree_name, child_element, indent_tab=DEFAULT_TAB, leading_tab="", debug_str=""):
    ...
    if nr_childs > 0:
        ...
        for child in tree[child_element]:
            res.append(
                tree2string(
                    child, tree_name, child_element, indent_tab, leading_tab + indent_tab, debug_str + tree_name + "."
                )
            )
...
def tree2file(fd, tree, tree_name, child_element, indent_tab=DEFAULT_TAB, leading_tab="", debug_str=""):
    ...
    if nr_childs > 0:
        ...
        for child in tree[child_element]:
            tree2file(
                fd, child, tree_name, child_element, indent_tab, leading_tab + indent_tab, debug_str + tree_name + "."
            )
