...
def exe_cmd(argv_func, argv=sys.argv):
    try:
        ...
        glideinMonitor.monitor(
            args["jid"],
            args["schedd_name"],
            args["pool_name"],
            args["timeout"],
            createCmdMonitorFile,
            argv_func(args["argv"]),
        )
...
def createCmdMonitorFile(
    ...
    if "USES_MONITOR_STARTD" in condor_status:
        ...
        monitor_startd = (
            False  # if not defined, assume the old operation mode (no monitor startd)
        )

    if monitor_startd:
        # job running in a different execute dir
        script_lines.append(
            "outdir=`ls -dlt ../../../execute* | tail -1 | awk '{print $9}'`"
        )
        script_lines.append(
            "(cd $outdir/dir*; if [ $? -eq 0 ]; then %s; else echo Internal error; fi)"
            % argv2cmd(argv)
        )
    else:
        ...
        script_lines.append(
            "(cd ../$outdir; if [ $? -eq 0 ]; then %s; else echo Internal error; fi)"
            % argv2cmd(argv)
        )

    return monitorScriptFromList(
        monitor_file_name, monitor_control_relname, script_lines
    )


# callback function for glideinMonitor.monitor
# executes the command
def createCmdMonitorFileSimple(
    monitor_file_name, monitor_control_relname, argv, condor_status, monitorVM
):
    script_lines = []
...
def createCmdMonitorFileScript(
    monitor_file_name, monitor_control_relname, script_list, condor_status, monitorVM
):
    return monitorScriptFromList(
        monitor_file_name, monitor_control_relname, script_list
    )
