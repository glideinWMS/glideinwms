...
def createCmdMonitorFile(monitor_file_name, monitor_control_relname, argv, condor_status, monitorVM):
    ...
    if monitor_startd:
        ...
        script_lines.append(
            "(cd $outdir/dir*; if [ $? -eq 0 ]; then %s; else echo Internal error; fi)" % argv2cmd(argv)
        )
    else:
...
def createCmdMonitorFileSimple(monitor_file_name, monitor_control_relname, argv, condor_status, monitorVM):
    script_lines = []
...
def createCmdMonitorFileScript(monitor_file_name, monitor_control_relname, script_list, condor_status, monitorVM):
