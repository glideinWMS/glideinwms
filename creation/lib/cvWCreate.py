...
def create_initd_startup(startup_fname,frontend_dir,glideinWMS_dir):
    ...
    try:
        ...
        fd.write("        else\n")
        fd.write("           has_arg=0\n")
        fd.write('           echo $"Usage: frontend_startup reconfig <fname> <writeback yes|no>"\n')
        fd.write("           exit 1\n")
        fd.write("        fi\n")
        ...
        fd.write("          fi\n")
        fd.write("        fi\n")
        fd.write('        "$glideinWMS_dir/creation/reconfig_frontend" -force_name "$frontend_name" -writeback "$writeback" -update_scripts "no" -xml $2\n')
        fd.write('      RETVAL=$?\n')
        ...
        fd.write("        else\n")
        fd.write("           has_arg=0\n")
        fd.write('           echo $"Usage: frontend_startup upgrade <fname>"\n')
        ...
        fd.write("          fi\n")
        fd.write("        fi\n")
        fd.write('        "$glideinWMS_dir/creation/reconfig_frontend" -force_name "$frontend_name" -writeback "yes" -update_scripts "yes" -xml $1\n')
        fd.write("        reconfig_failed=$?\n")
        fd.write('        echo -n "Reconfiguring the frontend"\n')
        fd.write("        test $reconfig_failed -eq 0 && success || failure\n")
        fd.write('          RETVAL=$?\n')
