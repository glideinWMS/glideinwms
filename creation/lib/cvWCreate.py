...
def create_initd_startup(startup_fname,frontend_dir,glideinWMS_dir, cfg_name):
    ...
    try:
        ...
        fd.write("          fi\n")
        fd.write("        fi\n")
        fd.write('        "$glideinWMS_dir/creation/reconfig_frontend" -force_name "$frontend_name" -writeback "$writeback" -update_scripts "no" -xml "$cfg_loc" -update_def_cfg "$update_def_cfg"\n')
        fd.write('      RETVAL=$?\n')
