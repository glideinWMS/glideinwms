...
def create_initd_startup(startup_fname, factory_dir, glideinWMS_dir, cfg_name):
    ...
    try:
        ...
        fd.write("          fi\n")
        fd.write("        fi\n")
        fd.write('        "$glideinWMS_dir/creation/reconfig_glidein" -force_name "$glidein_name" -writeback "$writeback" -update_scripts "no" -xml "$cfg_loc" update_def_cfg "$update_def_cfg" $force_delete $fix_rrd -comment "$comment"\n')
