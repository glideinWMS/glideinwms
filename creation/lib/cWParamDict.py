...
def add_file_unparsed(user_file, dicts, is_factory):
    ...
    if period_value > 0: # period has 0 as default (in dictionary definition). Should I still protect against it not being defined?
    ...
    if is_executable:  # a script
        ...
        dicts[file_list_idx].add_from_file(
            ...
            cWDictFile.FileDictFile.make_val_tuple(
                cWConsts.insert_timestr(relfname), file_type, prefix, time, period, priority, cond_download, tar_source, config_out, cond_attr
            ),
            absfname,
        )
        ...
        dicts[file_list_idx].add_from_file(
            relfname, cWDictFile.FileDictFile.make_val_tuple(cWConsts.insert_timestr(relfname),"wrapper", tar_source=user_file.tar_source), absfname
        )
        ...
        dicts[file_list_idx].add_from_file(
            relfname,
            cWDictFile.FileDictFile.make_val_tuple(
                cWConsts.insert_timestr(relfname), "untar", cond_download=cond_download, config_out=config_out, cond_attr=cond_attr, absdir_outattr=wnsubdir
            ),
            absfname,
        )
        ...
        dicts[file_list_idx].add_from_file(
            relfname, cWDictFile.FileDictFile.make_val_tuple(cWConsts.insert_timestr(relfname), "source", tar_source=tar_source, time=time, priority=priority), absfname
        )
        ...
        dicts[file_list_idx].add_from_file(
            relfname, cWDictFile.FileDictFile.make_val_tuple(cWConsts.insert_timestr(relfname), user_file.type, tar_source=tar_source, time=time, priority=priority), absfname
        )
