...
# Not used, left as example of configuration checks
#
# def check_attr(attr):
...
def main(conf_file):
    ...
    try:
        ...
        return "Config file not readable: %s" % conf_file
    except Exception:
        return "Error parsing config file: %s" % conf_file
    root = tree.getroot()
    fc_list = []
    ...
    for fc in fc_list:
        try:
            ...
            if (
                f_version.startswith("glideinWMS 3.10")
                or f_version.startswith("glideinWMS 3.9")
                or f_version.startswith("glideinWMS 3.8")
                # Not expecting any Factory older than 3.8
            ):
                all_3_11 = False
                break
        except (OSError, IndexError, AttributeError):
            mylog(
                "WARNING: Unable to retrieve Factory version for %s. Assuming it is compatible with all your Frontend and configuration."
                % fc
            )
    if all_3_11:
        return "All connected Factories are at least v3.11 - OK"
    # return check_config(root)
    return "Your Frontend (>=v3.11) requires all connected Factories to be at least v3.11"
