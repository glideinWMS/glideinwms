...
def get_main_dicts(submit_dir, stage_dir):
    ...
    main_dicts["at_file_list"] = cWDictFile.FileDictFile(
        ...
        fname_idx=cgWConsts.AT_FILE_LISTFILE,
    )
    main_dicts["after_file_list"] = cWDictFile.FileDictFile(
        stage_dir,
        cWConsts.insert_timestr(cgWConsts.AFTER_FILE_LISTFILE),
        fname_idx=cgWConsts.AFTER_FILE_LISTFILE,
    )
    return main_dicts
...
def save_entry_dicts(
    entry_dicts,  # will update in place, too
    entry_name,
        ...
        for user in list(user_dirs.keys()):
            self.add_dir_obj(
                cWDictFile.symlinkSupport(
                    user_dirs[user],
                    os.path.join(self.symlink_base_dir, "user_%s" % user),
                    dir_name,
                )
            )
