...
class Config(xmlConfig.DictElement):
    ...
    def set_submit_dir(self):
        if eval(self["factory_versioning"]):
            self.submit_dir = os.path.join(
                self.get_child("submit")["base_dir"],
    ...
    def set_monitor_dir(self):
        if eval(self["factory_versioning"]):
            self.monitor_dir = os.path.join(
                self.get_child("monitor")["base_dir"],
                "glidein_%s" % self["glidein_name"],
            )
        else:
    ...
    def set_log_dir(self):
        if eval(self["factory_versioning"]):
            self.log_dir = os.path.join(
                self.get_child("submit")["base_log_dir"],
                "glidein_%s" % self["glidein_name"],
            )
        else:
    ...
    def set_web_url(self):
        if eval(self["factory_versioning"]):
            self.web_url = os.path.join(
                self.get_child("stage")["web_base_url"],
                "glidein_%s" % self["glidein_name"],
            )
        else:
    ...
    def set_num_factories(self):
        if eval(self["factory_versioning"]):
            self.num_factories = os.path.join(
                self.get_child("submit")["num_factories"],
                "glidein_%s" % self["glidein_name"],
            )
        else:
