...
class Config(xmlConfig.DictElement):
    ...
    def set_submit_dir(self):
        if eval(self['factory_versioning']):
            self.submit_dir = os.path.join(self.get_child('submit')['base_dir'],
                                           "glidein_%s" % self['glidein_name'])
        else:
            self.submit_dir = self.get_child('submit')['base_dir']

    def set_stage_dir(self):
        if eval(self['factory_versioning']):
            self.stage_dir = os.path.join(self.get_child('stage')['base_dir'],
                                          "glidein_%s" % self['glidein_name'])
        else:
    ...
    def set_monitor_dir(self):
        if eval(self['factory_versioning']):
            self.monitor_dir = os.path.join(self.get_child('monitor')['base_dir'],
                                            "glidein_%s" % self['glidein_name'])
        else:
    ...
    def set_log_dir(self):
        if eval(self['factory_versioning']):
            self.log_dir = os.path.join(self.get_child('submit')['base_log_dir'],
                                        "glidein_%s" % self['glidein_name'])
        else:
    ...
    def set_client_log_dirs(self):
        ...
        for fe in self.get_child('security').get_child_list('frontends'):
            for sc in fe.get_child_list('security_classes'):
                self.client_log_dirs[sc['username']] = os.path.join(client_dir,
                                                                     "user_%s" % sc['username'],
                                                                     "glidein_%s" % glidein_name)
    ...
    def set_client_proxy_dirs(self):
        ...
        for fe in self.get_child('security').get_child_list('frontends'):
            for sc in fe.get_child_list('security_classes'):
                self.client_proxy_dirs[sc['username']] = os.path.join(client_dir,
                                                                       "user_%s" % sc['username'],
                                                                       "glidein_%s" % glidein_name)

    def set_web_url(self):
        if eval(self['factory_versioning']):
            self.web_url = os.path.join(self.get_child('stage')['web_base_url'],
                                        "glidein_%s" % self['glidein_name'])
        else:
    ...
    def set_num_factories(self):
        if eval(self['factory_versioning']):
            self.num_factories = os.path.join(self.get_child('submit')['num_factories'],
                                        "glidein_%s" % self['glidein_name'])
        else:
