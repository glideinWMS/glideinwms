            ...
            for (var etc = 0; etc < el.childNodes.length; etc++) {
                ...
                if (entry.nodeType == 1 && entry.nodeName == "entry") {
                    ...
                    for (var etgs = 0; etgs < entry.childNodes.length; etgs++) {
                        ...
                        if (
                            grps.nodeType == 1 &&
                            grps.nodeName == "monitorgroups"
                        ) {
                            for (
                                var etg = 0;
                                etg < grps.childNodes.length;
                                etg++
                            ) {
                                var grp = grps.childNodes[etg];
                                if (
                                    grp.nodeType == 1 &&
                                    grp.nodeName == "monitorgroup"
                                ) {
                                    var group_name =
    ...
    xmlhttp_descript.onreadystatechange = function () {
        if (xmlhttp_descript.readyState == 4) {
            ...
            for (var i = 0; i < factory_info[0].attributes.length; i++) {
                ...
                if (
                    factory_info[0].attributes[i].name == "MonitorDisplayText"
                ) {
            ...
            document.getElementById("pgtitle").innerHTML =
                page_title + " - " + glidein_name + "@" + factory_name;
            document.getElementById("brtitle").innerHTML =
                browser_title + " - " + glidein_name + "@" + factory_name;
