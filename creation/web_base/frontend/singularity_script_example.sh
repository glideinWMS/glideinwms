...
SCRIPT_NAME=singularity_script_example
...
# BOTH All code out here will run on the 1st invocation and also in the re-invocation within singularity
# $GWMS_SINGULARITY_REEXEC is used to discriminate that (nothing outside, 1 inside)

if [ -z "$GWMS_SINGULARITY_REEXEC" ]; then
    # OUTSIDE  All the code in here is running only during the forst invocation, ourside of Singularity
    ...
    #################
    #
    # Add here below your code that you want to run OUTSIDE Singularity
    #
    #################
    ...
    if [ $HAS_SINGULARITY -eq 0 ]; then
        ...
        info_dbg "Singularity disabled, quitting w/o running in Singularity"
        exit $EXIT_CODE
    fi
    ...
    if [ "x$HAS_SINGULARITY" = "x1" -a "xGWMS_SINGULARITY_PATH" != "x" ]; then
        ...
        if [ "x$CVMFS_REPOS_LIST" != "x" ]; then
            for x in $(echo $CVMFS_REPOS_LIST | sed 's/,/ /g'); do
                if eval "exec $holdfd</cvmfs/$x"; then
                    info "/cvmfs/$x exists and available"
                    let "holdfd=holdfd+1"
                else
                    info "/cvmfs/$x NOT available"
                    sleep $EXITSLEEP
                    exit 1
                fi
              done
        ...
        if [ "x$GWMS_SINGULARITY_IMAGE" = "x" ]; then 
           info "Error: If you get this error when you did not specify desired OS, your VO does not support any default image" 1>&2
           info "Error: If you get this error when you specified desired OS, your VO does not support that OS" 1>&2
           exit 1
        fi
    else  #if [ "x$HAS_SINGULARITY" = "x1" -a "xSINGULARITY_PATH" != "x" ];
        if ! echo "$GWMS_SINGULARITY_IMAGE" | grep ^"/cvmfs" >/dev/null 2>&1; then
            info "warning: $GWMS_SINGULARITY_IMAGE is not in /cvmfs area" 1>&2
            exit 1
        fi
    fi
    ...
    if [ ! -e "$GWMS_SINGULARITY_IMAGE" ]; then
        info "Error: $GWMS_SINGULARITY_IMAGE is not found" 1>&2
        exit 1
    fi
    ...
    # notably the singularity-script.sh (this script!)
    cp $0 .gwms-${SCRIPT_NAME}
    export TEST_JOB_SINGULARITY="/srv/.gwms-${SCRIPT_NAME}"
    ...
    info_dbg  exec "$GWMS_SINGULARITY_PATH" exec $GWMS_SINGULARITY_EXTRA_OPTS \
                               ...
                               --pwd /srv \
                               --contain --ipc --pid   \
                               "$GWMS_SINGULARITY_IMAGE"  $TEST_JOB_SINGULARITY $CMD_ARGS
...
# Still INSIDE Singularity (outside of it ends w/ the exec og Singularity or exit)

# Some las setup: modules and env, ...
...
#################
#
# Add here below your code that you want to run INSIDE Singularity
#
#################
