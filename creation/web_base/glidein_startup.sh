...
<OSGTestResult id=\"fetch_file_base\" version=\"4.3.1\">
  ...
  <detail>
    ...
    if [[ "${ffb_file_type}" = "exec" || "${ffb_file_type}" = "exec:"* ]]; then
        ...
        elif [[ "${ffb_target_fname}" = "cvmfs_unmount.sh" ]] || [[ -n "${cleanup_script}" && "${ffb_target_fname}" = ${cleanup_script} ]]; then  # cleanup_script global for simplicity
            # TODO: temporary OR checking for cvmfs_unmount.sh; to be removed after Bruno's ticket on cleanup [#25073]
