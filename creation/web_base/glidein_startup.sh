...
check_file_signature() {
    ...
    # for line 753: integer expression expected error in stderr file
    # echo "check_signature set to $check_signature"
    # removed double quotes around the check_signature variable for resolving the integer expression error
...
if [[ -z "$gwms_cvmfs_reexec" ]]; then
    ...
    #####################
    # Periodic execution support global variable
    ...
    export LAST_SCRIPT="$last_script"
...
do
    gs_id="$(echo "${gs_file_id}" |awk '{print $1}')"       # one of four possibilities
