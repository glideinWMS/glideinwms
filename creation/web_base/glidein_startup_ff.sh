...
if [[ "$status" -eq 0 ]]; then
    ...
    # for regular glidein invocation
...
<OSGTestResult id=\"fetch_file_base\" version=\"4.3.1\">
  ...
  <detail>
    ...
    if [[ "${ffb_file_type}" = "exec" || "${ffb_file_type}" = "exec:"* ]]; then
        ...
        elif [[ "${ffb_target_fname}" = "cvmfs_umount_ff.sh" ]] || [[ -n "${cleanup_script}" && "${ffb_target_fname}" = "${cleanup_script}" ]]; then  # cleanup_script global for simplicity
