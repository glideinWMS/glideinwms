...
class glideinMainDicts(cgWDictFile.glideinMainDicts):
    ...
    def populate(self, params=None):
        ...
        for pfile in params.files:
            add_file_unparsed(pfile, self.dicts)
    ...
    def save_pub_key(self):
        if self.params.security.pub_key == 'None':
            ...
            if not os.path.isfile(rsa_key_fname):
                ...
                key_obj = glideinwms_libs.pubCrypto.RSAKey()
...
class glideinDicts(cgWDictFile.glideinDicts):
    ...
    def local_populate(self, params):
        ...
        global_schedd_names = params.schedd_name.split(',')
        global_schedd_idx = 0
        for sub_name in self.sub_list:
...
def add_file_unparsed(file, dicts):
    ...
    if is_executable: # a script
        ...
        if wnsubdir == None:
            wnsubdir = relfname.split('.', 1)[0] # deafult is relfname up to the first .
...
def populate_factory_descript(work_dir, glidein_dict, active_sub_list, # will be modified
                              params):
