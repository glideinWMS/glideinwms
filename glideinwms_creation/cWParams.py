...
#
# File Version: 
#   $Id: cWParams.py,v 1.16.2.1 2010/09/08 03:29:59 parag Exp $
#
# Desscription:
...
class SubParams:
    ...
    def validate(self, base, path_text):
        for k in self.data.keys():
            if not base.has_key(k):
                ...
                defel = base[k]
                if isinstance(defel, glideinwms_libs.xmlParse.OrderedDict):
                    # subdictionary
                    self[k].validate(defel, "%s.%s" % (path_text, k))
                else:
                    # final element
                    defvalue, _, _, subdef = defel

                    if isinstance(defvalue, glideinwms_libs.xmlParse.OrderedDict):
                        ...
                        # list of elements
                        if isinstance(self.data[k], glideinwms_libs.xmlParse.OrderedDict):
        ...
        for k in defaults.keys():
            ...
            if isinstance(defel, glideinwms_libs.xmlParse.OrderedDict):
                ...
                if not self.data.has_key(k):
                    self.data[k] = glideinwms_libs.xmlParse.OrderedDict() # first create empty, if does not exist
                ...
                if isinstance(defvalue, glideinwms_libs.xmlParse.OrderedDict):
                    # dictionary el elements
                    if not self.data.has_key(k):
                        self.data[k] = glideinwms_libs.xmlParse.OrderedDict() # no elements yet, set and empty dictionary
                    else:
    ...
    def get_el(self, name):
        el = self.data[name]
        if isinstance(el, glideinwms_libs.xmlParse.OrderedDict):
            return self.__class__(el)
        elif type(el) == type([]):
            outlst = []
            for k in el:
                if isinstance(k, glideinwms_libs.xmlParse.OrderedDict):
...
class Params:
    def __init__(self, usage_prefix, src_dir, argv):
        ...
        self.defaults = glideinwms_libs.xmlParse.OrderedDict()
        self.init_defaults()
    ...
    def get_xml(self):
        old_default_ignore_nones = glideinwms_libs.xmlFormat.DEFAULT_IGNORE_NONES
        old_default_lists_params = glideinwms_libs.xmlFormat.DEFAULT_LISTS_PARAMS
        old_default_dicts_params = glideinwms_libs.xmlFormat.DEFAULT_DICTS_PARAMS
        glideinwms_libs.xmlFormat.DEFAULT_IGNORE_NONES = True
        # these are used internally, do not need to be ordered
        xml_format = self.get_xml_format()
        glideinwms_libs.xmlFormat.DEFAULT_LISTS_PARAMS = xml_format['lists_params']
        glideinwms_libs.xmlFormat.DEFAULT_DICTS_PARAMS = xml_format['dicts_params']
        # hack needed to make xmlFormat to properly do the formating
        old_DictType = types.DictType
        types.DictType = type(glideinwms_libs.xmlParse.OrderedDict())
        out = glideinwms_libs.xmlFormat.class2string(self.data, self.get_top_element())
        types.DictType = old_DictType
        glideinwms_libs.xmlFormat.DEFAULT_IGNORE_NONES = old_default_ignore_nones
        glideinwms_libs.xmlFormat.DEFAULT_LISTS_PARAMS = old_default_lists_params
        glideinwms_libs.xmlFormat.DEFAULT_DICTS_PARAMS = old_default_dicts_params
        return out
    ...
    def load_file(self, fname):
        if fname == "-":
            fname = sys.stdin
        try:
            self.data = glideinwms_libs.xmlParse.xmlfile2dict(fname, use_ord_dict=True)
...
class commentedOrderedDict(glideinwms_libs.xmlParse.OrderedDict):
    def __init__(self, dict=None):
        # cannot call directly the parent due to the particular implementation restrictions
        self._keys = []
        glideinwms_libs.xmlParse.UserDict.__init__(self, dict)
...
def defdict2string(defaults, indent, width=80):
    ...
    for k in keys:
        el = defaults[k]
        if not isinstance(el, glideinwms_libs.xmlParse.OrderedDict):
    ...
    for k in final_keys:
        el = defaults[k]
        if isinstance(el, glideinwms_libs.xmlParse.OrderedDict):  #sub-dictionary
