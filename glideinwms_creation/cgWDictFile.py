...
class CondorJDLDictFile(cWDictFile.DictFile):
    def __init__(self, jdl_dir, fname, sort_keys=False, order_matters=False, jobs_in_cluster=None,
                 fname_idx=None):      # if none, use fname
        cWDictFile.DictFile.__init__(self, jdl_dir, fname, sort_keys, order_matters, fname_idx)
...
class clientDirSupport(cWDictFile.simpleDirSupport):
    def __init__(self, user, client_dir, dir_name, privsep_mkdir=False):
        cWDictFile.simpleDirSupport.__init__(self, client_dir, dir_name)
        self.user = user
    ...
    def create_dir(self, fail_if_exists=True):
        ...
        if self.user == MY_USERNAME:
            ...
            try:
                ...
                glideinwms_libs.condorPrivsep.mkdir(base_dir, os.path.basename(self.dir), self.user)
            ...
            try:
                ...
                glideinwms_libs.condorPrivsep.execute(self.user, base_dir, '/bin/mkdir', ['mkdir', self.dir], stdout_fname=None)
            except glideinwms_libs.condorExe.ExeError, e:
                raise RuntimeError, "Failed to create %s dir (user %s): %s" % (self.dir_name, self.user, e)
            except:
                raise RuntimeError, "Failed to create %s dir (user %s): Unknown privsep error" % (self.dir_name, self.user)
    ...
    def delete_dir(self):
        ...
        if self.user == MY_USERNAME:
            ...
            try:
                ...
                glideinwms_libs.condorPrivsep.rmtree(base_dir, os.path.basename(self.dir))
            except glideinwms_libs.condorExe.ExeError, e:
            ...
            try:
                ...
                glideinwms_libs.condorPrivsep.execute(self.user, base_dir, '/bin/rm', ['rm', '-fr', self.dir], stdout_fname=None)
            except glideinwms_libs.condorExe.ExeError, e:
                raise RuntimeError, "Failed to remove %s dir (user %s): %s" % (self.dir_name, self.user, e)
            except:
                raise RuntimeError, "Failed to remove %s dir (user %s): Unknown privsep error" % (self.dir_name, self.user)
...
class chmodClientDirSupport(clientDirSupport):
    def __init__(self, user, client_dir, chmod, dir_name):
        clientDirSupport.__init__(self, user, client_dir, dir_name)
        self.chmod = chmod
    ...
    def create_dir(self, fail_if_exists=True):
        ...
        if self.user == MY_USERNAME:
            ...
            try:
                # use the execute command
                # do not use the mkdir one, as we do not need root privileges
                glideinwms_libs.condorPrivsep.execute(self.user, base_dir, '/bin/mkdir', ['mkdir', '-m', "0%o" % self.chmod, self.dir], stdout_fname=None)
            except glideinwms_libs.condorExe.ExeError, e:
                raise RuntimeError, "Failed to create %s dir (user %s): %s" % (self.dir_name, self.user, e)
            except:
                raise RuntimeError, "Failed to create %s dir (user %s): Unknown privsep error" % (self.dir_name, self.user)
...
class baseClientDirSupport(cWDictFile.multiSimpleDirSupport):
    def __init__(self, user, client_dir, dir_name='client'):
        cWDictFile.multiSimpleDirSupport.__init__(self, (), dir_name)
        self.user = user

        self.base_dir = os.path.dirname(client_dir)
        ...
        self.add_dir_obj(clientDirSupport(user, client_dir, dir_name))
