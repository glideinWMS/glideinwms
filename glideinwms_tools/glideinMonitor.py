...
def monitor(jid, schedd_name, pool_name,
    ...
    try:
        ...
        jid = glideinwms_libs.condorManager.condorSubmitOne(sname, schedd_name, pool_name)
        try:
            checkFile(mcname, schedd_name, pool_name, timeout, reschedule_freq=10)
            printFile(mfout, stdout_fd)
            printFile(mferr, stderr_fd)
        except:
            glideinwms_libs.condorManager.condorRemoveOne(jid, schedd_name, pool_name)
            raise
    finally:
...
def getRemoteVM(pool_name, schedd_name, constraint):
    cq = glideinwms_libs.condorMonitor.CondorQ(schedd_name=schedd_name, pool_name=pool_name)
...
def getMonitorVM(pool_name, jobVM):
    cs = glideinwms_libs.condorMonitor.CondorStatus(pool_name=pool_name)
...
def checkFile(fname, schedd_name, pool_name,
    ...
    while (time.time() < deadline):
        if (time.time() - last_reschedule) >= reschedule_freq:
            glideinwms_libs.condorManager.condorReschedule(schedd_name, pool_name)
            last_reschedule = time.time()
        time.sleep(1)
