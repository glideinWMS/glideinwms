...
def getCondorQCredentialList(factoryConfig=None):
    ...
    try:
        q_cred_list = condorMonitor.condorq_attrs(
            q_glidein_constraint,
            ["x509userproxy", "EC2AccessKeyId", "EC2SecretAccessKey"],
        )
    except:
...
def sanitizeGlideins(condorq, log=logSupport.log, factoryConfig=None):
    ...
    if len(unrecoverable_held_list) > 0:
        ...
        removeGlideins(
            condorq.schedd_name,
            unrecoverable_held_list_minus_forcex,
            force=False,
            log=log,
            factoryConfig=factoryConfig,
        )
...
def get_status_glideidx(el):
    ...
    return (
        el[factoryConfig.clusterid_startd_attribute],
    ...
    def __init__(
        ...
        for fe_sec_class in self.frontend_limits:
            ...
            fe_condorQ = condorMonitor.SubQuery(
                entry_condorQ,
                lambda d: (d[factoryConfig.frontend_name_attribute] == fe_sec_class),
            )
