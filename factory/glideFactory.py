...
def save_stats(stats, fname):
    ...
    util.file_pickle_dump(
        ...
        mask_exceptions=(
            ...
            "Saving of aggregated statistics failed: ",
        ),
    )
...
def main(startup_dir):
    ...
    try:
        if is_file_old(
            glideinDescript.default_rsakey_fname,
        ...
        if key_fname and cwd:
            logSupport.log.error(
                "Failed to load RSA key %s with current working direcotry %s",
                key_fname,
                cwd,
            )
            logSupport.log.error(
