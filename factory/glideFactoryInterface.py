...
def findWork(factory_name, glidein_name, entry_name, supported_signtypes,
    ...
    if supported_signtypes is not None:
        status_constraint += ' && stringListMember(%s%s,"%s")' % (factoryConfig.client_web_prefix, factoryConfig.client_web_signtype_suffix, str.join(supported_signtypes, ","))
...
class EntryClassad(classadSupport.Classad):
    ...
    def __init__(self, factory_name, glidein_name, entry_name,
        ...
        self.adParams[factoryConfig.factory_signtype_id] = "%s" % str.join(supported_signtypes, ',')
