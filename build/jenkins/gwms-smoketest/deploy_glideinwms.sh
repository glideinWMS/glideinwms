...
function is_vm_up() {
    ...
    while [ $retries -lt 30 ] ; do
        tmpout=`ssh -t -o StrictHostKeyChecking=no $fqdn hostname 2>&1`
        if [ "$tmpout" != "$fqdn" ] ; then
            echo -n "."
            sleep 30
...
function configure_fact() {
    ...
    if [ -f /var/lib/gwms-factory/condor/${base_condor_tarball} ]; then
       sed -e  s/__CONDOR_ARCH__/${condor_arch}/ -e s/__CONDOR_OS__/${condor_os}/ -e s/__CONDOR_VERSION__/${condor_version}/ ${AUTO_INSTALL_SRC_DIR}/Dev_Sites2.xml > ${ENTRIES_CONFIG_DIR}/Dev_Sites2.xml
       cd /var/lib/gwms-factory/condor/ 
       tar xvzf ${base_condor_tarball} 
    ...
    if [ -f /var/lib/gwms-factory/condor/${base_condor_tarball2} ]; then
       cd /var/lib/gwms-factory/condor/ 
       tar xvzf ${base_condor_tarball2} 
       sed -e  s/__CONDOR_ARCH2__/${condor_arch2}/ -e s/__CONDOR_OS2__/${condor_os2}/ -e s/__CONDOR_VERSION2__/${condor_version2}/ ${AUTO_INSTALL_SRC_DIR}/Dev_Sites3.xml > ${ENTRIES_CONFIG_DIR}/Dev_Sites3.xml
       sed -i -e s/__CONDOR_DIR2__/${base_condor_dir2}/ -e s/__CONDOR_ARCH2__/${condor_arch2}/ -e s/__CONDOR_OS2__/${condor_os2}/ -e s/__CONDOR_VERSION2__/${condor_version2}/ ${ENTRIES_CONFIG_DIR}/Condor_Tarballs.xml
    else
       sed -i -e /__CONDOR_DIR2__/d ${ENTRIES_CONFIG_DIR}/Condor_Tarballs.xml
    fi
...
add_dn_to_condor_mapfile "${dn_fmt}/CN=fermicloud046.fnal.gov" ce1
add_dn_to_condor_mapfile "${dn_fmt}/CN=fermicloud105.fnal.gov" ce2
add_dn_to_condor_mapfile "${dn_fmt}/CN=fermicloud127.fnal.gov" ce3
add_dn_to_condor_mapfile "${dn_fmt}/CN=fermicloud378.fnal.gov" ce4
...
dn_fmt='/DC=org/DC=incommon/C=US/ST=IL/L=Batavia/O=Fermi Research Alliance/OU=Fermilab'
...
if [ -n "${condor_tarball2}" ]; then
base_condor_tarball2=`basename ${condor_tarball2}` 
base_condor_dir2=`echo ${base_condor_tarball2} | sed s/.tar.gz//`
condor_version2=$(echo ${base_condor_tarball2} | sed s/condor-// | sed s/-.*//)
condor_os2=$(echo ${base_condor_tarball2} | sed s/.*_// | sed s/-.*//)
condor_arch2=$(echo ${base_condor_tarball2} | sed s/_${condor_os2}.*// | sed s/condor-${condor_version2}-//)
condor_os2=$(echo $condor_os2 | sed s/RedHat/rhel/g)
fi

#for some reason fermicloudui.fnal.gov is kicking me out, so 
#find one that doesnt...

SSH="ssh -t fcluigpvm02.fnal.gov"
...
source modules/create_fermicloud_vm2.sh --el ${el} --vm_template ${vm_template} --vm_name $fact_vm_name
fact_fqdn=$fqdn
fact_vmid=$vmid

source modules/create_fermicloud_vm2.sh --el ${el} --vm_template ${vm_template} --vm_name ${vofe_vm_name}
vofe_fqdn=$fqdn
vofe_vmid=$vmid
...
#is_vm_up ${fact_fqdn} && fact_fqdn_status="up"

#vofe_fqdn_status="down"
#is_vm_up ${vofe_fqdn} && vofe_fqdn_status="up"
...
fact_vm_dn="${dn_fmt}/CN=${fact_fqdn}"
vofe_vm_dn="${dn_fmt}/CN=${vofe_fqdn}"
r
...
if [ -n "${condor_tarball}" ]; then
    ssh -t  "root@${fact_fqdn}" 'mkdir -p /var/lib/gwms-factory/condor/' 
...
if [ -n "${condor_tarball2}" ]; then
    ssh -t  "root@${fact_fqdn}" 'mkdir -p /var/lib/gwms-factory/condor/' 
    if [ -f "${condor_tarball2}" ]; then
        scp "${condor_tarball2}" "root@${fact_fqdn}":/var/lib/gwms-factory/condor/
        if [ $? -ne 0 ]; then
            echo "failed to copy ${condor_tarball2} to ${fact_fqdn}, aborting installation"
            exit 1
        fi
    else
        cmd="/usr/bin/wget ${condor_tarball2} -O /var/lib/gwms-factory/condor/${base_condor_tarball2}"
        echo "wget command: ssh -t root@${fact_fqdn} $cmd"
        ssh -t  root@"${fact_fqdn}" "$cmd"
        #if [ $? -ne 0 ]; then
         #   echo "failed to load condor_tarball ${condor_tarball2} to ${fact_fqdn}, check URL"
         #   echo "aborting installation"
         #   exit 1
        #fi
    fi
...
echo "ssh -t root@${fact_fqdn} /tmp/fact_install.sh " > "/tmp/ssh_fact.${TS}.sh"
...
if grep -vs 'FACTORY VERIFICATION: SUCCESS'  "${FACT_LOG}.${TS}"  ; then
...
echo "ssh -t root@${vofe_fqdn} /tmp/vofe_install.sh" > "/tmp/ssh_frontend.${TS}.sh"
chmod +x "/tmp/ssh_frontend.${TS}.sh"
bash "/tmp/ssh_frontend.${TS}.sh" 2>&1 | tee -a  "${VOFE_LOG}.${TS}" 

echo "-------------------------- Frontend Deployment Completed ----------------------"

if grep -s 'FRONTEND VERIFICATION: SUCCESS' "${VOFE_LOG}.${TS}" ; then
