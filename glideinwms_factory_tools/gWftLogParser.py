...
import glideinwms_factory.glideFactoryLogParser

# get the list of jobs that were active at a certain time
def get_glideins(log_dir_name, date_arr, time_arr):
    glidein_list = []

    cldata = glideinwms_factory.glideFactoryLogParser.dirSummaryTimingsOutFull(log_dir_name)
    ...
    for glidein_el in glidein_data:
        glidein_id, _, runningStartTimeStr, lastTimeStr = glidein_el
        runningStartTime = glideinwms_libs.condorLogParser.rawTime2cTimeLastYear(runningStartTimeStr)
        if runningStartTime > ref_ctime:
            continue # not one of them, started after
        lastTime = glideinwms_libs.condorLogParser.rawTime2cTimeLastYear(lastTimeStr)
...
def get_glidein_logs_entry(factory_dir, entry, date_arr, time_arr, ext="err"):
    ...
    for glidein_id in glidein_list:
        glidein_log_file = "job.%i.%i." % glideinwms_libs.condorLogParser.rawJobId2Nr(glidein_id)
