...
import glideinwms_libs.rrdSupport
...
class MonitoringConfig:
    def __init__(self):
        ...
        self.rrd_obj = glideinwms_libs.rrdSupport.rrdSupport()
    ...
    def logCompleted(self, client_name, entered_dict):
        ...
        try:
            for job_id in job_ids:
                ...
                fd.write(("<job %37s %34s %22s %17s %17s %22s %24s>" % (('terminated="%s"' % glideinwms_libs.timeConversion.getISO8601_Local(now)),
    ...
    def write_file(self, relative_fname, input_str):
        """ 
...
####################################
def time2xml(the_time, outer_tag, indent_tab=glideinwms_libs.xmlFormat.DEFAULT_TAB, leading_tab=""):
    xml_data = {"UTC":{"unixtime":glideinwms_libs.timeConversion.getSeconds(the_time),
                     "ISO8601":glideinwms_libs.timeConversion.getISO8601_UTC(the_time),
                     "RFC2822":glideinwms_libs.timeConversion.getRFC2822_UTC(the_time)},
              "Local":{"ISO8601":glideinwms_libs.timeConversion.getISO8601_Local(the_time),
                       "RFC2822":glideinwms_libs.timeConversion.getRFC2822_Local(the_time),
                       "human":glideinwms_libs.timeConversion.getHuman(the_time)}}
    return glideinwms_libs.xmlFormat.dict2string(xml_data,
...
class condorQStats:
    ...
    def get_xml_data(self, indent_tab=glideinwms_libs.xmlFormat.DEFAULT_TAB, leading_tab=""):
        data = self.get_data()
        return glideinwms_libs.xmlFormat.dict2string(data,
    ...
    def get_xml_total(self, indent_tab=glideinwms_libs.xmlFormat.DEFAULT_TAB, leading_tab=""):
        total = self.get_total()
        return glideinwms_libs.xmlFormat.class2string(total,
                                      inst_name="total",
                                      indent_tab=indent_tab, leading_tab=leading_tab)
    ...
    def get_xml_updated(self, indent_tab=glideinwms_libs.xmlFormat.DEFAULT_TAB, leading_tab=""):
    ...
    def get_xml_downtime(self, leading_tab=glideinwms_libs.xmlFormat.DEFAULT_TAB):
        xml_downtime = glideinwms_libs.xmlFormat.dict2string({}, dict_name='downtime', el_name='', params={'status':self.downtime}, leading_tab=leading_tab)
        return xml_downtime
    ...
    def write_file(self):
        ...
        xml_str = ('<?xml version="1.0" encoding="ISO-8859-1"?>\n\n' +
                 ...
                 self.get_xml_updated(indent_tab=glideinwms_libs.xmlFormat.DEFAULT_TAB, leading_tab=glideinwms_libs.xmlFormat.DEFAULT_TAB) + "\n" +
                 self.get_xml_downtime(leading_tab=glideinwms_libs.xmlFormat.DEFAULT_TAB) + "\n" +
                 self.get_xml_data(indent_tab=glideinwms_libs.xmlFormat.DEFAULT_TAB, leading_tab=glideinwms_libs.xmlFormat.DEFAULT_TAB) + "\n" +
                 self.get_xml_total(indent_tab=glideinwms_libs.xmlFormat.DEFAULT_TAB, leading_tab=glideinwms_libs.xmlFormat.DEFAULT_TAB) + "\n" +
                 "</glideFactoryEntryQStats>\n")
...
class condorLogSummary:
    ...
    def get_xml_stats_data(self, indent_tab=glideinwms_libs.xmlFormat.DEFAULT_TAB, leading_tab=""):
    ...
    def get_xml_stats_total(self, indent_tab=glideinwms_libs.xmlFormat.DEFAULT_TAB, leading_tab=""):
...
class FactoryStatusData:
    ...
    def __init__(self):
        ...
        for rrd in rrd_list:
            self.data[rrd] = {}
        self.updated = time.time()
        self.tab = glideinwms_libs.xmlFormat.DEFAULT_TAB
    ...
    def fetchData(self, file_name, pathway, res, start, end):
        ...
        baseRRDSupport = glideinwms_libs.rrdSupport.rrdSupport()
        try:
            fetched = baseRRDSupport.fetch_rrd(pathway + file_name, 'AVERAGE', resolution=res, start=start, end=end)
        except:
            # probably not created yet
            glideFactoryLib.log_files.logDebug("Failed to load %s" % (pathway + file_name))
            return {}
    ...
    def average(self, a_list):
        try:
            if len(a_list) > 0:
                avg_list = sum(a_list) / len(a_list)
            else:
                avg_list = 0
            return avg_list
        except TypeError:
        ...
        for rrd in rrd_list:
            ...
            for res_raw in self.resolution:
                ...
                try:
                    ...
                    fetched_data = self.fetchData(rrd, pathway, rrd_res, start, end)
    ...
    def getXMLData(self, rrd):
        ...
        try:
            ...
            total_xml_str += (glideinwms_libs.xmlFormat.dict2string(total_data, dict_name='periods', el_name='period', subtypes_params={"class":{}}, indent_tab=self.tab, leading_tab=2 * self.tab) + "\n")
        ...
        for frontend in self.frontends:
            ...
            try:
                ...
                frontend_xml_str += (glideinwms_libs.xmlFormat.dict2string(frontend_data, dict_name='periods', el_name='period', subtypes_params={"class":{}}, indent_tab=self.tab, leading_tab=3 * self.tab) + "\n")
            except UnboundLocalError:
                glideFactoryLib.log_files.logDebug("FactoryStatusData:frontend_data: UnboundLocalError")
            except NameError:
                glideFactoryLib.log_files.logDebug("FactoryStatusData:frontend_data: NameError")
...
class Descript2XML:
    ...
    def frontendDescript(self, desc_dict):
        for key in self.frontend_blacklist:
            try:
                for frontend in desc_dict:
                    try:
                        del desc_dict[frontend][key]
                    except KeyError:
                        continue
            except RuntimeError:
                glideFactoryLib.log_files.logDebug("blacklist RuntimeError in frontendDescript")
        try:
            desc_str = glideinwms_libs.xmlFormat.dict2string(desc_dict, dict_name="frontends", el_name="frontend", subtypes_params={"class":{}}, leading_tab=self.tab)
            return desc_str + "\n"
        except RuntimeError:
            glideFactoryLib.log_files.logDebug("xmlFormat RuntimeError in frontendDescript")
            return

    def entryDescript(self, desc_dict):
        for key in self.entry_descript_blacklist:
            try:
                for entry in desc_dict:
                    try:
                        del desc_dict[entry]['descript'][key]
                    except KeyError:
                        continue
            except RuntimeError:
                glideFactoryLib.log_files.logDebug("blacklist RuntimeError in entryDescript")
        try:
            desc_str = glideinwms_libs.xmlFormat.dict2string(desc_dict, dict_name="entries", el_name="entry", subtypes_params={"class":{'subclass_params':{}}}, leading_tab=self.tab)
            return desc_str + "\n"
        except RuntimeError:
            glideFactoryLib.log_files.logDebug("xmlFormat RuntimeError in entryDescript")
            return

    def glideinDescript(self, desc_dict):
        w_dict = {}
        for key in self.glidein_whitelist:
            try:
                w_dict[key] = desc_dict[key]
            except KeyError:
                continue
        try:
            a = glideinwms_libs.xmlFormat.dict2string({'':w_dict}, dict_name="glideins", el_name="factory", subtypes_params={"class":{}})
            b = a.split("\n")[1]
            c = b.split('name="" ')
            desc_str = "".join(c)
            return desc_str + "\n"
        except SyntaxError:
            glideFactoryLib.log_files.logDebug("xmlFormat SyntaxError in glideinDescript")
            return
        except RuntimeError:
    ...
    def writeFile(self, path, input_str, singleEntry=False):
        if singleEntry:
            root_el = 'glideFactoryEntryDescript'
        else:
            root_el = 'glideFactoryDescript'
        xml_str = ('<?xml version="1.0" encoding="ISO-8859-1"?>\n\n<%s>\n%s\n%s</%s>' % (root_el, self.getUpdated(), input_str, root_el))
        #xml_str = ('<?xml version="1.0" encoding="ISO-8859-1"?>\n\n' +
        #           '<' + root_el + '>\n' + self.getUpdated() + "\n" + input_str +
        #           '</' + root_el + '>')
