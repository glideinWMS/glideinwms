...
class GlideinKey:
    ...
    def load(self, key_fname=None, recreate=False):
        ...
        if self.pub_key_type == 'RSA':
            ...
            self.rsa_key = glideinwms_libs.pubCrypto.RSAKey(key_fname=key_fname)
            ...
            self.pub_key_id = md5.new("".join((self.pub_key_type, self.pub_rsa_key.get()))).hexdigest()
            self.sym_class = glideinwms_libs.symCrypto.AutoSymKey
        else:
