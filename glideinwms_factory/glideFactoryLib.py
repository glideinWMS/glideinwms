...
class PrivsepDirCleanupWSpace(glideinwms_libs.logSupport.DirCleanupWSpace):
    def __init__(self,
        ...
        glideinwms_libs.logSupport.DirCleanupWSpace.__init__(self, dirname, fname_expression,
    ...
    def delete_file(self, fpath):
        if (self.username != None) and (self.username != MY_USERNAME):
            ...
            glideinwms_libs.condorPrivsep.execute(self.username, os.path.dirname(fpath), '/bin/rm', ['rm', fpath], stdout_fname=None)
        else:
...
class LogFiles:
    def __init__(self, log_dir, max_days, min_days, max_mbs):
        ...
        self.activity_log = glideinwms_libs.logSupport.DayLogFile(os.path.join(log_dir, "factory"), "info.log")
        self.warning_log = glideinwms_libs.logSupport.DayLogFile(os.path.join(log_dir, "factory"), "err.log")
        self.debug_log = glideinwms_libs.logSupport.DayLogFile(os.path.join(log_dir, "factory"), "debug.log")
        # no need to use the privsep version
        self.cleanupObjs = [glideinwms_libs.logSupport.DirCleanupWSpace(log_dir, "(factory\.[0-9]*\.info\.log)|(factory\.[0-9]*\.err\.log)|(factory\.[0-9]*\.debug\.log)",
...
def getCondorQData(entry_name,
    ...
    q = glideinwms_libs.condorMonitor.CondorQ(schedd_name)
...
def getQProxSecClass(condorq,
    ...
    entry_condorQ = glideinwms_libs.condorMonitor.SubQuery(condorq, lambda d:(d.has_key(csa_str) and (d[csa_str] == client_name) and
...
def getQStatus(condorq):
    qc_status = glideinwms_libs.condorMonitor.Summarize(condorq, hash_status).countStored()
    return qc_status

def getQStatusStale(condorq):
    qc_status = glideinwms_libs.condorMonitor.Summarize(condorq, hash_statusStale).countStored()
    return qc_status
...
def getCondorStatusData(entry_name, client_name, pool_name=None,
    ...
    status = glideinwms_libs.condorMonitor.CondorStatus(pool_name=pool_name)
...
def update_x509_proxy_file(entry_name, username, client_id, proxy_data):
    ...
    if username != MY_USERNAME:
        ...
        try:
            glideinwms_libs.condorPrivsep.execute(username, factoryConfig.submit_dir, os.path.join(factoryConfig.submit_dir, 'update_proxy.py'), ['update_proxy.py'], update_proxy_env)
...
def keepIdleGlideins(client_condorq, client_int_name,
    ...
    condorq = glideinwms_libs.condorMonitor.SubQuery(client_condorq, lambda d:(d[factoryConfig.x509id_schedd_attribute] == x509_proxy_identifier))
...
def extractStaleUnclaimed(q, status):
    ...
    # find out if any "Idle" glidein is running instead (in condor_status)
    global factoryConfig
    sgroup = glideinwms_libs.condorMonitor.Group(status, lambda el:get_status_glideidx(el), group_unclaimed)
...
def extractJobId(submit_out):
    ...
    raise glideinwms_libs.condorExe.ExeError, "Could not find cluster info!"
        ...
        while (nr_submitted < nr_glideins):
            ...
            if username != MY_USERNAME:
                ...
                try:
                    ...
                    #                                 ['--']+params_arr,
                    #                                 exe_env)
                    submit_out = glideinwms_libs.condorPrivsep.execute(username, factoryConfig.submit_dir,
                                                     os.path.join(factoryConfig.submit_dir, factoryConfig.submit_fname),
                                                     [factoryConfig.submit_fname, entry_name, client_name, x509_proxy_security_class, x509_proxy_identifier, "%i" % nr_to_submit, glidein_rsl, ] +
                                                     client_web_arr +
                                                     ['--'] + params_arr,
                ...
                try:
                    submit_out = glideinwms_libs.condorExe.iexe_cmd('export X509_USER_PROXY=%s;./%s "%s" "%s" "%s" "%s" %i "%s" %s -- %s' % (x509_proxy_fname, factoryConfig.submit_fname, entry_name, client_name, x509_proxy_security_class, x509_proxy_identifier, nr_to_submit, glidein_rsl, client_web_str, params_str))
                except glideinwms_libs.condorExe.ExeError, e:
                    submit_out = []
...
def removeGlideins(schedd_name, jid_list, force=False):
    ...
    for jid in jid_list:
        ...
        try:
            glideinwms_libs.condorExe.exe_cmd("condor_rm", "%s %li.%li" % (schedd_str, jid[0], jid[1]))
            ...
            if force == True:
                try:
                    ...
                    glideinwms_libs.condorExe.exe_cmd("condor_rm", "-forcex %s %li.%li" % (schedd_str, jid[0], jid[1]))
                except glideinwms_libs.condorExe.ExeError, e:
...
def releaseGlideins(schedd_name, jid_list):
    ...
    for jid in jid_list:
        ...
        try:
            glideinwms_libs.condorExe.exe_cmd("condor_release", "%s %li.%li" % (schedd_str, jid[0], jid[1]))
