...
def get_production_ress_entries(server, ref_dict_list):
    ...
    condor_obj = glideinwms_libs.condorMonitor.CondorStatus(pool_name=server)
...
def get_production_bdii_entries(server, ref_dict_list):
    ...
    bdii_obj = glideinwms_libs.ldapMonitor.BDIICEQuery(server)
    bdii_obj.load()
...
def infosys_based(entry_name, opt_dict, infosys_types):
    ...
    import glideinwms_creation.cgWDictFile
    import glideinwms_creation.cgWConsts

    for entry in config_els.keys():
        infosys_fd = glideinwms_creation.cgWDictFile.InfoSysDictFile(glideinwms_creation.cgWConsts.get_entry_submit_dir('.', entry), glideinwms_creation.cgWConsts.INFOSYS_FILE)
        infosys_fd.load()
