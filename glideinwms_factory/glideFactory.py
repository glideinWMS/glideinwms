...
def aggregate_stats(in_downtime):
    try:
        glideFactoryMonitorAggregator.aggregateStatus(in_downtime)
    except:
        # protect and report
        tb = traceback.format_exception(sys.exc_info()[0], sys.exc_info()[1],
                                        sys.exc_info()[2])
        glideFactoryLib.log_files.logDebug("aggregateStatus failed: %s" % "".join(tb))
    ...
    try:
        ...
        tb = traceback.format_exception(sys.exc_info()[0], sys.exc_info()[1],
                                        sys.exc_info()[2])
        glideFactoryLib.log_files.logDebug("aggregateLogStatus failed: %s" % "".join(tb))
    ...
    try:
        ...
        tb = traceback.format_exception(sys.exc_info()[0], sys.exc_info()[1],
                                        sys.exc_info()[2])
        glideFactoryLib.log_files.logDebug("aggregateRRDStats failed: %s" % "".join(tb))
