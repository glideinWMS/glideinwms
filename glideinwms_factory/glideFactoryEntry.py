...
import glideinwms_libs.glideinWMSVersion
...
def find_and_perform_work(in_downtime, glideinDescript, frontendDescript, jobDescript, jobAttributes, jobParams):
    ...
    try:
        ...
        tb = traceback.format_exception(sys.exc_info()[0], sys.exc_info()[1],
                                        sys.exc_info()[2])
        glideFactoryLib.log_files.logWarning("getCondorQData failed, traceback: %s" % ''.join(tb))
        # protect and exit
        return 0
            ...
            for i in range(nr_x509_proxies):
                ...
                try:
                    ...
                    tb = traceback.format_exception(sys.exc_info()[0], sys.exc_info()[1],
                                                    sys.exc_info()[2])
                    glideFactoryLib.log_files.logWarning("Failed to update x509_proxy_%i using usename %s for client %s, skipping request" % (i, x509_proxy_username, client_int_name))
                    glideFactoryLib.log_files.logDebug("Failed to update x509_proxy_%i using usename %s for client %s: Exception %s" % (i, x509_proxy_username, client_int_name, ''.join(tb)))
                    continue # skip request
...
def main(parent_pid, sleep_time, advertize_rate, startup_dir, entry_name):
    ...
    if not (entry_name in ','.split(glideinDescript.data['Entries'])):
    ...
    try:
        ...
        glideFactoryInterface.factoryConfig.glideinwms_version = glideinwms_libs.glideinWMSVersion.GlideinWMSDistro(dir, os.path.join(dir, 'etc/checksum.factory')).version()
    except:
        tb = traceback.format_exception(sys.exc_info()[0], sys.exc_info()[1],
                                        sys.exc_info()[2])
