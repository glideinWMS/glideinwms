...
class CredentialsPlugin(ABC):
    ...
    def generate_credentials(self, dynamic_snapshot: Optional[str] = None, **kwargs):
        """
        Generate all credentials that are generators

        Args:
            dynamic_snapshot (str, optional): creates a snapshot of the generated credentials
            **kwargs: keyword arguments to be passed to the generator
        """

        for cred in self.cred_list:
            if isinstance(cred, DynamicCredential):
                cred.generate(dynamic_snapshot, **kwargs)

    def generate_parameters(self, dynamic_snapshot: Optional[str] = None, **kwargs):
        """
        Generate all parameters that are generators

        Args:
            dynamic_snapshot (str, optional): creates a snapshot of the generated parameters
            **kwargs: keyword arguments to be passed to the generator
        """

        for param in self.params_dict.values():
            if isinstance(param, DynamicParameter):
                param.generate(dynamic_snapshot, **kwargs)

    def update_usermap(self, condorq_dict, condorq_dict_types, status_dict, status_dict_types):
        return

    @abstractmethod
    def get_credential(self, cred_id: str, dynamic_snapshot: Optional[str] = None) -> Optional[Credential]:
        pass

    @abstractmethod
    def get_parameter(self, param_name: ParameterName, dynamic_snapshot: Optional[str] = None) -> Optional[Parameter]:
        pass

    @abstractmethod
    def get_credentials(
        self, credential_type=None, trust_domain=None, credential_purpose=None, dynamic_snapshot: Optional[str] = None
    ) -> List[Credential]:
        pass

    @abstractmethod
    def get_parameters(self, dynamic_snapshot: Optional[str] = None) -> Mapping[ParameterName, Parameter]:
        pass
...
class CredentialsBasic(CredentialsPlugin):
    ...
    def get_credential(self, cred_id, dynamic_snapshot=None):
        cred = self.cred_dict.get(cred_id, None)
        if dynamic_snapshot and isinstance(cred, DynamicCredential):
            cred_snapshot = cred.get_snapshot(dynamic_snapshot)
            cred = cred_snapshot if cred_snapshot else cred
        return cred if cred else None

    def get_parameter(self, param_name: ParameterName, dynamic_snapshot: Optional[str] = None) -> Optional[Parameter]:
        param = self.params_dict.get(param_name, None)
        if dynamic_snapshot and isinstance(param, DynamicParameter):
            param_snapshot = param.get_snapshot(dynamic_snapshot)
            param = param_snapshot if param_snapshot else param
    ...
    def get_credentials(
        ...
        dynamic_snapshot: Optional[str] = None,
    ) -> List[Credential]:
        ...
        Args:
            ...
            dynamic_snapshot (str, optional): get credentials from a dynamic snapshot if available
        ...
        for cred in self.cred_list:
            ...
            if credential_purpose and cred.purpose not in credential_purpose:
                continue
            if isinstance(cred, DynamicCredential) and dynamic_snapshot:
                cred_snapshot = cred.get_snapshot(dynamic_snapshot)
                cred = cred_snapshot if cred_snapshot else cred
    ...
    def get_parameters(self, dynamic_snapshot: Optional[str] = None) -> Mapping[ParameterName, Parameter]:
        ...
        Args:
            dynamic_snapshot (str, optional): get parameters from a dynamic snapshot if available
        ...
        params_dict = {}
        for param in self.params_dict.values():
            if isinstance(param, DynamicParameter) and dynamic_snapshot:
                param_snapshot = param.get_snapshot(dynamic_snapshot)
                param = param_snapshot if param_snapshot else param
