...
class MultiAdvertizeWork:
    ...
    def createAdvertizeWorkFile(self, factory_pool, params_obj, key_obj=None, file_id_cache=None):
        ...
        for key in params_obj.glidein_params_to_encrypt.keys():
            if key.endswith(".idtoken"):
                try:
                    idtoken_str = params_obj.glidein_params_to_encrypt[key]
                    idtoken = create_credential(
                        idtoken_str, cred_type=CredentialType.IDTOKEN, purpose=CredentialPurpose.CALLBACK
                    )
                    payload_creds.append(idtoken)
