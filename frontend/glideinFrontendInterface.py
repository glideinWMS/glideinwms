...
class MultiAdvertizeWork:
    ...
    def createAdvertizeWorkFile(self, factory_pool, params_obj, key_obj=None, file_id_cache=None): 
        ...
        if total_nr_credentials == 0:
            logSupport.log.debug("Error total_nr_credentials is 0") 
        ...
        if nr_credentials == 0:
            logSupport.log.debug("Total nr_credentials is 0")
            logSupport.log.debug("Factory auth %s" % factory_auth)
            raise NoCredentialException
        ...
        for i in range(nr_credentials):
            ...
            try:
                ...
                if True: # for historical reasons... to preserve indentation
                    ...
                    logSupport.log.debug("Checking Credential type %s ..."%(credential_el.type))
                    ...
                    logSupport.log.debug("credential_el.project_id %s " % credential_el.project_id)
                    if credential_el.project_id:
                        logSupport.log.debug("Adding project_id %s to the glideins params to be encrypted" % credential_el.project_id)
