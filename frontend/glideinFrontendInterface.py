    ...
    def createAdvertiseWorkFile(self, factory_pool, params_obj, key_obj=None, file_id_cache=None):
        ...
        payload_creds = [
            ...
            for cred in self.descript_obj.credentials_plugin.get_credentials(
                ...
                snapshot=params_obj.glidein_name,
            )
        ]
        ...
        security_params = [
            param
            for param in self.descript_obj.credentials_plugin.get_parameters(snapshot=params_obj.glidein_name).values()
        ]
        ...
        for request_cred in self.request_credentials:
            ...
            glidein_params_to_encrypt["RequestCredentials"] = pickle.dumps(
                [
                    self.descript_obj.credentials_plugin.get_credential(
                        request_cred.credential.id, snapshot=params_obj.glidein_name
                    )
                ]
            )
