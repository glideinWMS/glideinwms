...
class TestInfosysLib(unittest.TestCase):
    ...
    def test_query_ress(self):
        ...
        condorExe.init()
        self.assertTrue(
            condorExe.condor_bin_path is not None and condorExe.condor_sbin_path is not None)
        condorExe.set_path(
            condorExe.condor_bin_path,
    ...
    def test_query_teragrid(self):
        ...
        # There is no input to the function since it is specific to the url
        # requested
    ...
    @unittest.skip('fails, dont know if should be fixed')
    def test_parse_entries(self):
        ...
        self.assertEqual(
            entries['valid_entry']['gatekeeper'],
        ...
        self.assertEqual(
            entries['valid_entry']['glidein_attrs'], {
                'CONDOR_ARCH': 'default', 'GLEXEC_BIN': 'NONE', 'GLIDEIN_Site': 'VE1'})

        self.assertEqual(entries['valid_entry2']['name'], 'valid_entry2')
        self.assertEqual(entries['valid_entry2']['enabled'], 'False')
        self.assertEqual(
            entries['valid_entry2']['gatekeeper'],
        ...
        missing_ref_id_entries = parse_entries(
            config_dom, skip_missing_ref_id=True, skip_disabled=False)
        self.assertEqual(len(missing_ref_id_entries), 1)

        # Test skip disabled
        disabled_entries = parse_entries(
            config_dom,
            skip_missing_ref_id=False,
            skip_disabled=True)
        ...
        disabled_entries = parse_entries(
            config_dom, skip_missing_ref_id=True, skip_disabled=True)
    ...
    def test_parse_factory_schedds(self):
        ...
        self.assertEqual(schedds,
                         ['schedd_glideins1@test.gov',
                          'schedd_glideins2@test.fnal.gov'])
        os.remove('valid.xml')
    ...
    @unittest.skip('fails, dont know if worth fixing')
