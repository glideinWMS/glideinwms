...
class TestDictFileTwoKeys(unittest.TestCase):
    """test class for DictFileTwoKeys from glideinwms/creation/lib/cWDictFile.py
    TODO  File cWDictFile.py is 1700 lines long and contains too
    many classes.  It should be split up
    """
    
    def test___getitem__(self):
        self.assertEqual("'True'", self.dict_file.__getitem__('GLIDECLIENT_Group_Start'))

    def setUp(self):
        self.dict_file = DictFileTwoKeys(dir="fixtures/frontend/group_group1", fname="attrs.cfg",sort_keys=True)
        self.dict_file.load()
            ...
            for cf in range(0,2):
                for ck in range(0,2):
                    self.assertTrue(self.dict_file.is_equal(other,
                                                            compare_dir=bool(cd),
                                                            compare_fname=bool(cf),
                                                            compare_keys=bool(ck)))
        ...
        for cd in range(0,2):
            for cf in range(0,2):
                ...
                self.assertTrue(self.dict_file.is_equal(other,
                                                        ...
                                                        compare_keys=ck),
                                                        "%s=%s self=\n%s other=\n%s"%\
                                                        ('assertTrue compare_keys',
                                                        ck,self.dict_file.save_into_str(),
                                                         other.save_into_str()))
                ck = True
                self.assertFalse(self.dict_file.is_equal(other,
                                                        compare_dir=bool(cd),
    ...
    def test_remove(self):
        #TODO: seems like fail_if_missing is correct unlike this classes base class
        #
        key = 'GLIDECLIENT_Group_Start'
    ...
    def test_save_to_load_from_str(self):
        other = copy.deepcopy(self.dict_file)
        d_str = self.dict_file.save_into_str(sort_keys=False, set_readonly=False, reset_changed=False, want_comments=False)
