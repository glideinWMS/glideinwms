...
class MonitoringConfig:
    ...
    def write_file(self, relative_fname, contents_str):
        fname = os.path.join(self.monitor_dir, relative_fname)
        #print "Writing "+fname
        fd = open(fname + ".tmp", "w")
        try:
            fd.write(contents_str + "\n")
        finally:
            fd.close()
...
class groupStats:
    ...
    def get_xml_updated(self, indent_tab=glideinwms_libs.xmlFormat.DEFAULT_TAB, leading_tab=""):
        xml_updated = {"UTC":{"unixtime":glideinwms_libs.timeConversion.getSeconds(self.updated),
                            "ISO8601":glideinwms_libs.timeConversion.getISO8601_UTC(self.updated),
                            "RFC2822":glideinwms_libs.timeConversion.getRFC2822_UTC(self.updated)},
                     "Local":{"ISO8601":glideinwms_libs.timeConversion.getISO8601_Local(self.updated),
                              "RFC2822":glideinwms_libs.timeConversion.getRFC2822_Local(self.updated),
                              "human":glideinwms_libs.timeConversion.getHuman(self.updated)}}
        return glideinwms_libs.xmlFormat.dict2string(xml_updated,
