...
#
# File Version: 
#   $Id: glideinFrontendMonitorAggregator.py,v 1.10.8.6 2011/07/07 16:49:36 sfiligoi Exp $
#
# Description:
...
def aggregateStatus():
    ...
    for group in monitorAggregatorConfig.groups:
        ...
        try:
            group_data = glideinwms_libs.xmlParse.xmlfile2dict(status_fname)
        except IOError:
    ...
    xml_str = ('<?xml version="1.0" encoding="ISO-8859-1"?>\n\n' +
             ...
             glideinwms_libs.xmlFormat.dict2string(status["groups"], dict_name="groups", el_name="group",
             ...
             glideinwms_libs.xmlFormat.dict2string(global_fact_totals, dict_name="factories", el_name="factory",
                                   subtypes_params={"class":{"subclass_params":{"Requested":{"dicts_params":{"Parameters":{"el_name":"Parameter",
...
def get_xml_updated(when, indent_tab=glideinwms_libs.xmlFormat.DEFAULT_TAB, leading_tab=""):
    xml_updated = {"UTC":{"unixtime":glideinwms_libs.timeConversion.getSeconds(when),
                        "ISO8601":glideinwms_libs.timeConversion.getISO8601_UTC(when),
                        "RFC2822":glideinwms_libs.timeConversion.getRFC2822_UTC(when)},
                 "Local":{"ISO8601":glideinwms_libs.timeConversion.getISO8601_Local(when),
                          "RFC2822":glideinwms_libs.timeConversion.getRFC2822_Local(when),
                          "human":glideinwms_libs.timeConversion.getHuman(when)}}
