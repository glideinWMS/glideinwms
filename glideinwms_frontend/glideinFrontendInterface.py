...
#
# File Version: 
#   $Id: glideinFrontendInterface.py,v 1.47.2.13 2011/06/20 15:14:08 parag Exp $
#
# Description:
...
import glideinwms_libs.pubCrypto
import glideinwms_libs.symCrypto
...
class MultiExeError(glideinwms_libs.condorExe.ExeError):
...
def findGlideins(factory_pool, factory_identity,
    ...
    status = glideinwms_libs.condorMonitor.CondorStatus("any", pool_name=factory_pool)
...
class FactoryKeys4Advertize:
    def __init__(self,
        ...
        if glidein_symKey == None:
            glidein_symKey = glideinwms_libs.symCrypto.SymAES256Key()
    ...
    def encrypt_hex(self, enc_str):
        return self.glidein_symKey.encrypt_hex(enc_str)
...
def exe_condor_advertise(fname, command, pool, is_multi=False):
    return glideinwms_libs.condorManager.condorAdvertise(fname, command, frontendConfig.advertise_use_tcp, is_multi, pool)
